...
exports.run = async (client, message, args) => {
  ...
  if (!member)
    return message.reply(
      "Bir kullanıcı belirt g!profil @Gnarge veya g!profil <Kullanıcı_ID> "
    );
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addField(
      "Üye bilgisi:",
      `**Kullanıcı İsmi:** ${member.displayName}\n**Katılım Tarihi:** ${moment
        .utc(member.joinedAt)
        .format("Do MMMM YYYY")} - ${checkDays(
        member.joinedAt
      )} \n**Rolleri:** ${member.roles.cache
        .sort((b, a) => {
    ...
    .addField(
      ...
      `\n**Tag**: ${member.user.tag}\n**ID:** ${
        member.user.id
      }\n**Kuruluş Tarihi**: ${moment
        .utc(user.createdAt)
        .format("Do MMMM YYYY")} - ${checkDays(user.createdAt)}`,
      false
    )
    .setFooter(
